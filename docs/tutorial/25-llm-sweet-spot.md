...
One line. The agent reads the decorator, sees `audited()` isn't there yet, adds `.chain(audited())`. Done. No signals to discover. No middleware to untangle. No mixins to trace. Everything about this entity — its CRUD, its auth, its audit trail — is right here, on the entity.
...
**Compositionality.** Adding a concern means appending to a tuple. `.chain(paginated())` doesn't modify the existing derivation — it concatenates new steps. `.chain(audited())` doesn't reach into the CRUD handlers — it wraps them. Each transform reads only the effects it cares about. Concerns don't interfere because they don't share mutable state.
