...
class TestMemoryRelationalFetch:
    ...
    @pytest.mark.asyncio
    async def test_fetch_many_all(self, prov):
        result = await prov.fetch_many(relational(User))
        assert len(result) == 3

    @pytest.mark.asyncio
    async def test_fetch_many_filter(self, prov):
    ...
    @pytest.mark.asyncio
    async def test_fetch_one(self, prov):
...
class TestMemoryRelationalMutations:
    ...
    @pytest.mark.asyncio
    async def test_update_not_found(self, prov):
        with pytest.raises(ValueError, match="not found"):
            await prov.update(ALICE)

    @pytest.mark.asyncio
    async def test_delete(self, prov):
        await prov.insert(ALICE)
    ...
    @pytest.mark.asyncio
    async def test_delete_where(self, prov):
        await prov.insert(ALICE)
        await prov.insert(BOB)
    ...
    @pytest.mark.asyncio
    async def test_upsert_requires_key_fn(self):
...
class TestMemoryRelationalAggregate:
    ...
    @pytest.mark.asyncio
    async def test_aggregate_with_filter(self, prov):
        q = (
            relational(User)
            .filter(lambda u: u.active == True)
    ...
    @pytest.mark.asyncio
    async def test_multiple_aggregates(self, prov):
        q = relational(User).aggregate(
...
class TestAggregateEmptyDataset:
    ...
    @pytest.mark.asyncio
    async def test_min_max_empty(self):
        prov = MemoryRelationalProvider[User]()
        q = relational(User).aggregate(
            lo=lambda u: u.balance.min(),
            hi=lambda u: u.balance.max(),
...
class TestMemoryRelationalNextId:
    @pytest.mark.asyncio
    async def test_sequence_next_id(self):
    ...
    @pytest.mark.asyncio
    async def test_prefixed_next_id(self):
        prov = MemoryRelationalProvider[User](
    ...
    @pytest.mark.asyncio
    async def test_uuid_next_id(self):
        import uuid
    ...
    @pytest.mark.asyncio
    async def test_no_next_id_raises(self):
        prov = MemoryRelationalProvider[User]()
...
class TestMemoryRelationalAtomic:
    @pytest.mark.asyncio
    async def test_atomic_context(self):
    ...
    @pytest.mark.asyncio
    async def test_atomic_serializes_concurrent(self):
        import asyncio
...
class TestKVQuerySetImmutability:
    def test_kv_queryset_immutable(self):
        q1 = kv(User, key=lambda u: u.id)
        q2 = q1.get("alice")
        assert q1.op is None  # unchanged
        assert q2.op is not None

    def test_kv_queryset_returns_new(self):
        q1 = kv(User, key=lambda u: u.id)
        q2 = q1.set("alice", User(id="a", name="Alice", balance=0, active=True))
        q3 = q1.delete("bob")
        assert q1 is not q2
        assert q1 is not q3
        assert q2 is not q3
...
class TestMemoryKV:
    ...
    @pytest.mark.asyncio
    async def test_set_and_get(self, prov, users_kv):
    ...
    @pytest.mark.asyncio
    async def test_get_missing(self, prov, users_kv):
        result = await prov.get(users_kv.get("nobody"))
        assert result is None

    @pytest.mark.asyncio
    async def test_delete(self, prov, users_kv):
        await prov.set(users_kv.set("alice", ALICE))
    ...
    @pytest.mark.asyncio
    async def test_keys(self, prov, users_kv):
    ...
    @pytest.mark.asyncio
    async def test_put(self, prov, users_kv):
        await prov.set(users_kv.put(ALICE))
        result = await prov.get(users_kv.get(ALICE.id))
        assert result is not None
        assert result.name == "alice"

    @pytest.mark.asyncio
    async def test_put_extracts_key(self, prov, users_kv):
        await prov.set(users_kv.put(BOB))
        assert await prov.get(users_kv.get(BOB.id)) is not None
        assert await prov.get(users_kv.get(ALICE.id)) is None

    @pytest.mark.asyncio
    async def test_set_with_ttl_accepted(self, prov, users_kv):
    ...
    @pytest.mark.asyncio
    async def test_wrong_op_raises(self, prov, users_kv):
...
class TestMemoryKVIntKeys:
    ...
    @pytest.mark.asyncio
    async def test_int_key_preserved(self, prov, users_kv):
    ...
    @pytest.mark.asyncio
    async def test_int_key_exists(self, prov, users_kv):
    ...
    @pytest.mark.asyncio
    async def test_int_key_delete(self, prov, users_kv):
        await prov.set(users_kv.set(1, ALICE))
