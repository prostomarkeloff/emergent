"""Compiler toolkit — extract typed (trigger, handler) pairs from wire primitives.

The Trigger × Codec product space defines all possible exposures.
``scan`` and ``scan_stack`` extract the region a compiler claims:

    from emergent.wire._scan import scan, scan_stack
...
def scan_endpoint(
    ...
    for trg, cdc in endp.exposures:
        if not isinstance(trg, trigger):
            continue
        if codec is not None and not isinstance(cdc, codec):
            continue
        results.append((trg, Handler(codec=cdc, runner=endp.runner)))
    return results
